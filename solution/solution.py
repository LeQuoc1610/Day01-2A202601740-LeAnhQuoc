...
def run_assistant(
    ...
    while True:
        ...
        turns += 1
        prompt_tokens = count_tokens(user_msg, model=OPENAI_MODEL)
        completion_tokens = count_tokens(reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def format_comparison_table(results: list[dict]) -> str:
        def shorten(text: str) -> str:
            s = str(text)
            return s if len(s) <= 40 else f"{s[:37]}..."
    ...
    for r in results:
        prompt = pick(r, ["prompt", "prompt_text"])
        gpt4o_resp = pick(r, ["gpt4o_answer", "gpt4o_response", "gpt4o"])
        mini_resp = pick(r, ["mini_answer", "mini_response", "mini"])
        gpt4o_lat = pick(r, ["gpt4o_time", "gpt4o_latency", "gpt4o_time_seconds"], 0)
        mini_lat = pick(r, ["mini_time", "mini_latency", "mini_time_seconds"], 0)
        ...
        rows.append(
            " | ".join(
                [
                    ...
                    shorten(gpt4o_resp),
                    shorten(mini_resp),
                    f"{float(gpt4o_lat):.2f}s",
                    f"{float(mini_lat):.2f}s",
                ]
            )
        )
    return "\n".join(rows)
